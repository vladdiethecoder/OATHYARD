...
for file in \
  ...
  tools/native_roster_showcase.sh \
...
mkdir -p artifacts/verify_a artifacts/verify_b
...
log_run "$final_dir/native_roster_showcase.log" ./tools/native_roster_showcase.sh artifacts/native_roster/verify
...
log_run "$final_dir/runtime_3d_audit.log" ./tools/audit_3d_runtime.sh artifacts/runtime_3d/verify assets/runtime_manifest.json artifacts/native_combat/verify/native_combat_render_manifest.json
log_run "$final_dir/renderer_target_audit.log" ./tools/renderer_target_audit.sh artifacts/renderer_target/verify

grep -q '"renderer": "native-software-3d"' artifacts/native_combat/verify/native_combat_render_manifest.json
grep -q '"native_3d_runtime_geometry": true' artifacts/native_combat/verify/native_combat_render_manifest.json
grep -q '"projection_uses_z_depth": true' artifacts/native_combat/verify/native_combat_render_manifest.json
...
grep -q '"game_is_3d": true' artifacts/native_roster/verify/native_roster_showcase_manifest.json
...
echo "verify passed: 3D renderer-only presentation path with PBR material evidence"
