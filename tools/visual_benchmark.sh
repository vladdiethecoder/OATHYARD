...
if [[ -n "$unit083_matrix" && -f "$unit083_matrix" ]]; then
  python3 tools/unit083_native_asset_matrix.py validate --mode benchmark --out "$out" --matrix "$unit083_matrix"
  exit $?
fi
...
if [[ -n "$unit082_matrix" && -f "$unit082_matrix" ]]; then
  python3 tools/unit082_visual_evidence.py validate-capture-matrix --mode benchmark --out "$out" --matrix "$unit082_matrix"
  exit $?
fi
