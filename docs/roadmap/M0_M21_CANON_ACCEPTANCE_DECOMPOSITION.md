...
- Raw X11/XWayland, SVG, PPM, low-poly glTF, diagnostic contact sheets, software-raster captures, and metadata-only checks are local verification evidence only; they do not prove high-fidelity product presentation.
...
Final readiness-sensitive evidence must also name the final replay hash, content hash, package SHA-256, deterministic A/B comparison result, visual contact-sheet path, skipped checks, blocker classes, and current false external readiness flags.
