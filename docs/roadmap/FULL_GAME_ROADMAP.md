...
- Scripted duel parser, simultaneous commit/reveal, cost breakdowns, contact packets, injury/capability deltas, replay hashes, report, trace, and fight-film camera/shot manifest.
...
- Verify all six default fighter/loadout families through source-backed runtime glTF manifests now, and through manifest-backed native 3D renderer captures only after that capture path exists.
...
- Native graphics window is required for product-presentation completion; until a continuous player-facing 3D renderer is verified, nonvisual manifests/reports can prove only pipeline boundaries and blocked status.
