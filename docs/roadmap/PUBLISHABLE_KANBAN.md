...
- Linux `.desktop` and scalable non-native diagram icon metadata now validate locally and are packaged; AppStream/metainfo remains intentionally blocked while `LICENSE` is `PENDING / UNLICENSED`.
...
| D-BASE-023 | Linux desktop entry metadata validates and is packaged locally. | `packaging/linux/io.oathyard.OATHYARD.desktop`; `tools/desktop_metadata.sh`; `tools/package.sh`; `tools/smoke_package.sh`. | `./tools/desktop_metadata.sh artifacts/desktop_metadata/verify`; package smoke validates the packaged `.desktop` file and launches through the packaged `.desktop` `Exec=oathyard` path. AppStream/metainfo remains blocked by `LICENSE` pending/unlicensed status. |
...
| D-BASE-034 | Visual evidence reducer indexes source-run and package-smoke visual status without fallback visual proof. | `tools/visual_evidence_index.sh`; `tools/verify.sh`; `tools/publishable_gate.sh`; `artifacts/visual_evidence/verify/visual_evidence_manifest.json`. | `./tools/visual_evidence_index.sh artifacts/visual_evidence/verify`; report must keep owner visual acceptance `false`, truth mutation `none`, and block until manifest-backed native 3D captures are present. `./tools/publishable_gate.sh` must write timestamp-local visual status reports and failed-artifact lists, not standalone rollup images. |
...
| N-RENDER-001 | Native presentation | Define production native renderer acceptance target before implementation: platform(s), backend candidate list, frame timing, capture method, visual bar. | Covered locally by `D-BASE-030`; current blocked native-renderer status 3D evidence is still not production renderer completion. | `docs/decisions/0002-native-presentation-target.md` plus `./tools/renderer_target_audit.sh artifacts/renderer_target/verify`; next pull is implementation depth in `X-RENDER-001`. |
...
| X-RENDER-001 | Renderer | Build a manifest-backed native 3D renderer/camera path for production readability: arena, fighters, weapon arcs, contact, bind/guard/stagger/collapse/injury/recovery states. | `N-RENDER-001`, `X-TRUTH-001`, `D-BASE-005`. | Current evidence is limited to truth/replay/status manifests and runtime glTF geometry with nonzero Z depth; remaining evidence needs production renderer completeness, player-facing native 3D captures with renderer/asset/camera/replay metadata, and owner visual acceptance kept separate. |
...
| ID | Card | Done evidence |
| --- | --- | --- |
| B-RENDER-001 | Move from blocked status manifests to a continuous native 3D render loop with deterministic replay-driven captures. | Current native combat render writes truth-read-only blocked status only; remaining work is a player-facing runtime loop with manifest-backed native 3D captures, persistent controls, and owner visual acceptance kept separate. |
| B-RENDER-002 | Implement production arena, fighters, weapons, armor silhouettes from source assets. | Current truth/content evidence records source-backed weapon reach/mass, armor coverage, and runtime mesh/glTF references for canonical scenario loadouts; remaining evidence requires production renderer completeness and native 3D capture parity. |
| B-RENDER-003 | Render readable contact states: hit, bind, guard, stagger, collapse, injury, recovery. | Current truth/replay data identifies hit/contact, guard/bind, parry, weapon arc, armor/material solve, injury/capability, grip loss, recovery, near miss, and stagger/collapse-risk moments; remaining evidence requires production-quality native 3D visual treatment. |
...
| B-REPLAY-002 | Fight-film camera manifests, native 3D player capture, and native shot stepping generated only from trace. | `D-BASE-016`, `D-BASE-017`; next step is richer player-state presentation through the native 3D renderer. |
| B-REPLAY-003 | Build native 3D capture comparison tooling. | Before/after capture manifest comparison plus frame annotations sourced from native 3D captures only. |
| B-REPLAY-004 | Add export bundle: replay + trace + reports + hash manifest. | Covered by `D-BASE-024`; remaining work is richer player-facing export UX and optional archive packaging after renderer/UI depth improves. Native 3D captures may be bundled only when they are manifest-backed renderer evidence. |
...
| B-QA-002 | Native visual acceptance pack. | Manifest-backed native 3D captures/video/screens with owner checklist; owner marks accepted or rejected. |
...
| E-PLATFORM-001 | Current native presentation is Linux blocked native-renderer status only. | Incompatible platform breadth. | Non-Linux native paths return `BLOCKED`; current package is `oathyard-linux-x86_64`. | Decide target breadth: Linux-only first, Steam Deck, Windows, or cross-platform backend research. |
...
- native 3D capture manifest path or explicit blocked status;
