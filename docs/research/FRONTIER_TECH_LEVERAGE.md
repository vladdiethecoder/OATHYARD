...
- Current blocked native-renderer status, non-native diagram, non-native frame, low-poly glTF, debug labels, wireframes, and non-native local raster captures are debug-local evidence only.
...
- Fallback plan: current blocked native-renderer status/non-native frame/non-native local raster evidence remains Tier 0 debug-local verification; custom OpenGL/Vulkan/direct native renderer with documented approximations remains the lowest-dependency production path candidate; if proprietary SDK terms, hardware limits, package impact, or visual artifacts fail, disable the candidate and keep `production_renderer_complete:false`.
...
The production renderer must replace or supersede native-status/non-native frame/debug rendering with native high-fidelity 3D evidence at 1920x1080+ minimum. It must support skinned fighters, layered armor/clothing, high-quality weapons, OATHYARD arena environments, PBR/equivalent materials, dynamic lights, shadows, GI/reflection solution or documented approximation, atmosphere, cinematic replay cameras, performance instrumentation, and strict truth isolation.

Do not call any of these high fidelity: non-native frame line art, non-native diagram timelines, debug overlays, wireframes, cubes/capsules/primitives, untextured meshes, software silhouettes, or screenshots without loaded production assets.
