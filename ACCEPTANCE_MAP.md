...
V0/V1 visual-fidelity escalation state: `./tools/visual_gap_audit.sh artifacts/visual_review/latest` classifies 8104 current visual artifacts as failing baseline/debug-local evidence and writes `artifacts/visual_review/latest/v0_current_visual_baseline_contact_sheet.png`. `docs/decisions/0009-production-renderer-selection.md` selects Bevy/wgpu as the next renderer spike path and records that ADR 0008's raw OpenGL spike is superseded for V1 selection unless explicitly revived. `/usr/bin/blender` is currently blocked by a MaterialX ABI/symbol failure (`undefined symbol: _ZTVN17MaterialX_v1_39_46OutputE`), so the production source-asset pipeline is blocked until a working DCC/source-authoring route is installed and verified.
...
| PBR material evidence | Source-backed PBR/equivalent material profiles cover weapons, armor, arenas, and fighters with albedo, roughness/metallic, normal/height, edge wear, dirt, blood/wetness, cloth grain, steel scratches, leather strain, stone dust, stitching, hair/skin variation, and material IDs; replay-derived material event captures remain presentation-only and prove material maps do not affect truth hashes. | `assets_src/materials/pbr_materials.oysrc`, `assets/materials/pbr_surface_manifest.json`, `./tools/pbr_materials.sh examples/duels/basic_oathyard.duel artifacts/pbr_materials/verify`, `artifacts/pbr_materials/verify/pbr_material_manifest.json`, `artifacts/pbr_materials/verify/pbr_material_response_sheet.ppm` |
| Asset budgets/previews | Runtime mesh/glTF/preview, source bytes, audio events, VFX events, vertices, indices, triangles, material counts, primitive counts, and the current 3D baseline are measured against local regression budgets while external validation and owner acceptance stay false. Source-backed asset previews are rendered into a local manifest/contact sheet without high-fidelity or owner-acceptance claims. | `./tools/asset_budget_audit.sh artifacts/asset_budget/verify`, `./tools/render_asset_previews.sh artifacts/asset_previews/verify`, `artifacts/asset_budget/verify/asset_budget_report.md`, `artifacts/asset_previews/verify/asset_preview_report.md` |
...
| Native roster 3D showcase | All six default fighter/loadout families render as deterministic native-software 3D PPM frames from source-backed runtime glTF after content hashes, with contact-sheet/report/manifest evidence and no owner visual acceptance claim. | `./tools/native_roster_showcase.sh artifacts/native_roster/verify`, `artifacts/native_roster/verify/native_roster_showcase_manifest.json`, `artifacts/native_roster/verify/native_roster_showcase_contact_sheet.svg` |
| Native combat render | Native capture consumes replay/duel result after hashes, reconstructs source-backed silhouettes from canonical scenario loadouts, verifies active weapon/armor/arena runtime mesh/glTF/preview references, projects generated extruded 3D glTF triangle geometry into native frames with integer presentation coordinates and Z-depth oblique projection, writes a 12-frame inspectable combat state sequence, writes 21 replay-derived motion frames, renders a 42-frame X11 playback-loop final capture, renders a 120-frame truth-rate native live loop with five PPM sample captures, writes third-person and first-person software-rasterized 3D mesh viewport PPMs, writes a 21-frame replay-derived software 3D mesh sequence from runtime glTF after truth hashes using depth-sorted filled triangles, and verifies 1280x720/1280x800 captures plus visual audit/contact-sheet evidence. | `./tools/native_combat_render.sh examples/duels/basic_oathyard.duel artifacts/native_combat/verify`, `artifacts/native_combat/verify/native_combat_render_manifest.json`, `artifacts/native_combat/verify/native_combat_visual_audit.md` |
| Visual evidence reducer | Source-run and package-smoke visual artifacts are reduced into one deterministic manifest/report/contact sheet with hashes and a failed-artifact triage file, while owner visual acceptance remains false. | `./tools/visual_evidence_index.sh artifacts/visual_evidence/verify`, `artifacts/visual_evidence/verify/visual_evidence_manifest.json`, `artifacts/visual_evidence/verify/failed_visual_artifacts.txt` |
| Native presentation target | ADR and audit define the production renderer acceptance target, validate current raw X11/PPM local evidence including the UI-authored game-flow software-3D evidence preview, all-six-family native roster 3D showcase, the 120-frame native live loop, first-person/third-person software 3D mesh viewport captures, and the replay-derived 21-frame software 3D mesh sequence, while requiring `game_is_3d: true` and explicitly keeping production 3D gameplay completion, production renderer completion, and owner visual acceptance false. | `docs/decisions/0002-native-presentation-target.md`, `./tools/renderer_target_audit.sh artifacts/renderer_target/verify` |
